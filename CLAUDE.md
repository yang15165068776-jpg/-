# JSJG Character OS v8.8.1 — ITRL + NCE 物理状态追踪 + 叙事连贯性

> 最后更新：2026-07-11（v8.8.1 部署上线）
...
v8.8: 🧠 ITRL v1 — 内心活动渲染层（1新文件 + 4集成文件）
        【核心】innerThoughtRenderer.js (~180行) — 角色内心活动生成引擎:
          静态规则(~1.2KB): 核心原则+允许/禁止模式+内心活动类型+输出比例+格式规则
          动态指令(~0.3KB/turn): 好感度阶段→内心主题映射（5档：计算→分类→混合→冲突→自我欺骗）
          人格色彩: pursuer/confrontational/aloof/gentle 四种内心风格
        【缓存】静态规则→characterPrefix.js缓存（~1.2KB，一次性token成本）
        【注入】coordinator.js: 动态指令→系统消息（NDOS后，offensiveTail前）
        【RQA增强】rqa.js: 新增⑫ InnerThoughtBoundary检查（检测替玩家内心）
        【架构】CEK v4 → ITRL → Narrative Output → RQA（11维审计）
        【原则】内心活动=Private Character State，不存入Memory/Graph/Canon
v8.6: 🧠 疯批人格引擎 v3 — 关系系统自我黑化引擎（1文件）
...
⬜ v8.9: Emotion Drift System（情绪漂移 — 角色情绪随时间自然波动）
```
...
✅ v8.8: 🧠 ITRL v1 — 内心活动渲染层（角色内在世界+好感度动态+记忆隔离）
⬜ v8.9: Emotion Drift System（情绪漂移 — 角色情绪随时间自然波动）
⬜ v8.10: Timeline Forking（时间线分叉 — 多结局支持）
