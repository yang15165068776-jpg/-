...
3. **独立审稿 AI（reviewReply）**— flash 模型生成后审稿：结尾平淡？语气变温和？特质缺失/洁净化？不通过带反馈重试（最多2次）
...
- 动态函数，每轮注入角色阶段实时校准（阶段名/好感度/禁止行为/语言样本/玩家策略）
- 之前包含三黑名单+最终出口检查，现已由 Reviewer 接管

### 独立审稿（`reviewReply`）
- **模型**: `deepseek-v4-flash`
- **检查项**: ①结尾平淡 ②语气变温和 ③特质缺失/洁净化
- **成本**: ~80 token/次
- **失败处理**: API失败静默放行，审稿不通过最多重试2次
...
- **主AI**: 生成故事（承担所有system prompt指令）
- **Reviewer**: `deepseek-v4-flash`，生成后审稿
  - 检查①结尾平淡 ②语气变温和 ③特质缺失/洁净化
  - 不通过→`lastReviewFeedback`注入→重试（最多2次）
  - 失败静默放行
- **Token节省**: 主AI system prompt减掉行为警察块（由审稿接管），预计每轮省50%+
