# JSJG Character OS v8.4 — Narrative Director OS (NDOS)

> 最后更新：2026-06-27（v8.4 自主性五层栈：AIIS + ANDS + DAS + DCS + NDOS）
...
└────────────┬────────────────────────────┘
             ↓
┌─────────────────────────────────────────┐
│  🎬 v8.4 自主性五层栈（Autonomy Stack） │
│                                         │
│  🧠 AIIS  — 角色想发消息（Daily自主）    │  autonomousInitiativeSystem.js
│  🎭 ANDS — 角色想抢剧情（Drama角色主动） │  autonomousNarrativeDrive.js
│  🌋 DAS  — 世界制造戏剧（剧情自动驾驶）  │  dramaAutopilot.js
│  🎛️ DCS  — 导演控制全局（五维爽感控制）  │  dramaControlSystem.js
│  🎬 NDOS — 导演拍连续剧（统一导演大脑）  │  narrativeDirectorOS.js
└────────────┬────────────────────────────┘
             ↓
┌─────────────────────────────────────────┐
│  🎬 Render Layer                        │
...
### 运行时编排器（Runtime Orchestrator）— v8.4 Pipeline

```
每一轮 NOS 运行循环（v8.4 完整版）：
  ① INPUT_PARSE      — 分类玩家输入
  ② CONSTITUTION     — 构建 CCL 宪法层
  ③ FACT_SYNC        — Fact Ledger 同步 + 注入
  ④ STATE_UPDATE     — USK 状态快照
  ⑤ RELATIONSHIP     — ARSL + Agency + World Engine tick
  ⑤.5 AIIS_TICK      — 🧠 日常自主消息调度（MotivationField → Intent → Burst）
  ⑤.6 ANDS_TICK      — 🎭 剧情角色主动性（AutonomyScore → NarrativeIntent → Schedule）
  ⑤.7 DAS_TICK       — 🌋 世界自动驾驶（Tension→Scene→Conflict→Interrupt）
  ⑤.8 DCS_DIRECT     — 🎛️ 导演控制（Spotlight/Pacing/爽点/Branch限流）
...
│   │
│   │  # ═══ v8.4 自主性五层栈 ═══
│   ├── autonomousInitiativeSystem.js  # 🧠 AIIS — 日常自主消息调度（MotivationField+Intent+Burst+Filter）
│   ├── autonomousNarrativeDrive.js    # 🎭 ANDS — 剧情角色主动性（AutonomyScore+WorldAwareness+Initiative）
│   ├── dramaAutopilot.js              # 🌋 DAS — 剧情自动驾驶（Tension/Scene/Conflict/Pressure/Interrupt）
│   ├── dramaControlSystem.js          # 🎛️ DCS — 导演控制系统（Spotlight/Pacing/爽点/Tension/Branch）
│   └── narrativeDirectorOS.js         # 🎬 NDOS — 统一导演大脑（5决策+Scene Card输出）
│
├── state/
...
| 剧情模式角色"被动等玩家" | 系统只有 prompt 层面的主动性提示，没有独立决策系统 | v8.4 自主性五层栈：AIIS(消息) + ANDS(行动) + DAS(世界) + DCS(控制) + NDOS(导演) |
