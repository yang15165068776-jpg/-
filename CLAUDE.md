...
## 剧情模式指令架构（buildGMPrompt）

### 三层防线（反RLHF漂移）
1. **创作自由基线**（最高优先级）— 露骨性描写/血腥暴力授权 + 反洁净化封印（"回避写=比写错更严重"）
2. **反驯化封印 + 反洁净化封印**（Section 0 末尾）— 列举AI默认叙事惯性（6条温和化+6条洁净化）声明无效
3. **独立审稿 AI（reviewReply）**— flash 模型生成后七项审稿（结尾/语气/特质/角色偏离/心理密度/自主性/情绪张力），不通过直接改写完整回复（不再触发 Writer 重试）

### System Prompt 结构（`buildGMPrompt`）
0. **创作自由基线** — 成人向黑暗叙事授权，反洁净化封印强制要求
...
3. **世界观**
4. **可攻略角色** — 好感度阶段精简（当前阶段完整展开，其余仅名称范围）+ 增减规则
5. **NPC设定**
6. **GM角色调度规则** — 角色自主生命线（玩家=闯入事件，非存在理由）、冲突与屈服
7. **思考指令**（`<think>...</think>`）
8. **写作风格** — 字数≥400/600、心理活动四层公式、动作/对话规则、结尾开放张力节点+自检
   - 8b. **情绪爆发模板** — 触发条件+四层写法公式+自由叙事情绪动线

### 每条用户消息注入（`buildUserWrapper`）
- 动态函数，每轮注入**角色阶段实时校准**（阶段名/好感度/禁止行为/语言样本/玩家策略）
- **已瘦身**: 三黑名单+最终出口检查已移交 Reviewer，只保留~150 token（阶段校准+2行写作提醒）
- 之前每条 ~700 token，瘦身后每条 ~150 token

### 独立审稿（`reviewReply`）
- **模型**: `deepseek-v4-flash`，max_tokens=4096
- **定位**: 成人向黑暗小说编辑，R18/暴力/堕落是正常内容，不改弱只改强
- **七项检查**: ①结尾平淡 ②语气变温和 ③特质缺失/洁净化 ④角色偏离（对照语言样本） ⑤心理密度（碎片/潜台词/身体背叛/每300字） ⑥自主性（推自己议程 vs 纯响应玩家） ⑦情绪张力
- **通过**: 返回原版回复
- **不通过**: Reviewer 直接输出完整改写版（`【修改后回复】`），保留原文优点只修问题；不再触发 Writer 重试
- **成本**: ~5,000 token/次（input 1,200 + output 4,000），flash 模型极低价，远低于 Writer 重试成本
- **失败处理**: API 失败静默放行，改写版过禁止词检查后直接返回
...
### 心理活动四层公式（Section 8）
1. **碎片意识** — 一闪而过的念头，不完整/不连贯/自相矛盾，不超过两行
2. **潜台词裂缝** — 说出口vs心里想的距离，"随你。"——他没有说：别走。
3. **身体背叛** — 生理反应泄露不肯承认的情绪
4. **意识流独白**（情绪激烈时触发）— 5-10行跳跃/重复/自我拉扯/无结论/戛然而止
- 节奏规则：动作段→心理层、对话前后必有心理层、结尾心理必须开放、密度≥2处/300字

### 情绪爆发模板（Section 8b）
- **触发**: 禁区被触碰/压抑超两轮/剧烈预期落差/执念相关出现
...
## 独立审稿系统（reviewReply）— 双模型自修复架构
- 替代旧版张力裁判（`judgeTension`）+ Writer 重试循环
- **主AI**: 生成故事（system prompt 保留角色/阶段/写作风格/情绪模板），只调用一次
- **Reviewer**: `deepseek-v4-flash`，生成后七项审稿
  - ①结尾平淡 ②语气变温和 ③特质缺失/洁净化
  - ④角色偏离（对照语言样本和禁止行为）
  - ⑤心理密度（碎片/潜台词/身体背叛/每300字≥2处）
  - ⑥自主性（推自己的议程 vs 纯响应玩家）
  - ⑦情绪张力（心跳加速/冲突拉扯/不确定性）
  - 通过→返回原版回复
  - 不通过→直接输出完整改写版（`【修改后回复】`），不再触发 Writer 重试
  - 定位为成人向编辑，不改弱只改强
- **Writer 重试**: 仅违禁词命中时触发（保留），审稿不通过不再触发 Writer 重试
- **Token 节省**: 每次避免 Writer 重试省 ~几万 token 上下文（流式长回复），Reviewer 改写多花 ~3,400 output token，净省巨大
