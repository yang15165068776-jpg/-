...
3. **独立审稿 AI（reviewReply）**— flash 模型生成后六项审稿（结尾/语气/特质/角色偏离/心理密度/自主性），不通过带具体修改方案重试（最多2次）
...
### 独立审稿（`reviewReply`）
- **模型**: `deepseek-v4-flash`，max_tokens=500
- **定位**: 成人向黑暗小说编辑，R18/暴力/堕落是正常内容，不改弱只改强
- **六项检查**: ①结尾平淡 ②语气变温和 ③特质缺失/洁净化 ④角色偏离（对照语言样本） ⑤心理密度（碎片/潜台词/身体背叛/每300字） ⑥自主性（推自己议程 vs 纯响应玩家）
- **通过**: 可附带润色后的结尾
- **不通过**: 输出具体修改方案（含改写示例），注入 `lastReviewFeedback` 触发重试
- **成本**: ~1,700 token/次（input 1,200 + output 500），flash 模型极低价
- **失败处理**: API 失败静默放行，重试最多 2 次
...
## 独立审稿系统（reviewReply）— 双模型架构
- 替代旧版张力裁判（`judgeTension`）
- **主AI**: 生成故事（system prompt 保留角色/阶段/写作风格/情绪模板）
- **Reviewer**: `deepseek-v4-flash`，生成后六项审稿
  - ①结尾平淡 ②语气变温和 ③特质缺失/洁净化
  - ④角色偏离（对照语言样本和禁止行为）
  - ⑤心理密度（碎片/潜台词/身体背叛/每300字≥2处）
  - ⑥自主性（推自己的议程 vs 纯响应玩家）
  - 不通过→输出具体修改方案→`lastReviewFeedback`注入→重试（最多2次）
  - 通过时可附带润色结尾
  - 定位为成人向编辑，不改弱只改强
- **Token 对比**: 主 AI 省掉 buildUserWrapper 行为警察（~550 token/轮），由审稿接管。每轮净省 ~5,500 token（20轮上下文）
