...
### Reviewer 强化架构（`reviewReply`）
- **签名**: `reviewReply(character, affections, userInput, writerReply, apiKey)`
...
```
【零：阶段行为容器——最高优先级】
  数据驱动，不套通用模板。强制引用 coreState/playerStrategy/languageSamples/forbiddenBehaviors。
  强化前先回答三问：核心状态？玩家策略？语言样本底色？
  例：状态"冷漠戒备"→推冷酷攻击、状态"受伤退后"→推内伤卑微、状态"渴望占有"→推占有欲

【写作铁律：直白粗粝，拒绝文艺腔——同等优先级】
  封杀六类文艺写法：比喻/排比/诗意留白/抽象情绪词/哲学感叹/唯美意象
  要求：短句碎句、心理用身体写、对话像真人说话、不比喻不抒情不升华
  自检：读起来像文学杂志→重写；像被拽着领子按进场景→对了

【一：情绪张力强化】推到极限，身体先行→行动越界→语言残缺→残留未平
【二：意识流独白注入】8-12行跳跃/拉扯/重复/自我否定
【三：潜台词裂缝加深】"随你。"——他没有说：别走。
【四：感官细节强化】身体是情绪的容器，不是情绪的标签
【五：黑暗特质具象化——反洁净化强制】
  封杀六种洁净化：浪子没碰她/滥交者改过自新/暴力只是吓唬/性用省略号/物化内心尊重/堕落者仍有善良
  强化策略：撕开回避处、性写器官动作、暴力写伤口疼痛、脏到读者不舒服=做对了
```
