# JSJG Character OS v8.5 — Cached Prefix Architecture + Save Isolation

> 最后更新：2026-06-28（v8.5 缓存前缀架构+角色同质化+存档隔离+长上下文状态锁）
...
### 运行时编排器（Runtime Orchestrator）— v8.5 Pipeline

```
每一轮 NOS 运行循环（v8.5 完整版）：
  ① INPUT_PARSE      — 分类玩家输入
  ①.5 CHAR_PREFIX    — 🗃️ 缓存前缀检测（阶段变化时重建）
