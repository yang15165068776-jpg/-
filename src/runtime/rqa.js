  ...
  for (const rc of rcList) {
    ...
    if (rc.affectionEnabled && rc.affectionStages?.length > 0) {
      ...
      if (currentStage) {
        ...
        if (currentStage.forbiddenBehaviors?.length > 0) {
          const fbs = currentStage.forbiddenBehaviors.filter(b => b.trim()).slice(0, 8)
          if (fbs.length > 0) {
            rules.push(`  ✗ 严禁：${fbs.map(b => b.trim()).join(' / ')}`)
          }
        }
        if (currentStage.languageSamples?.length > 0) {
          const samples = currentStage.languageSamples.filter(s => s.trim()).slice(0, 5)
          if (samples.length > 0) {
