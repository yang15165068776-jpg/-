...
function buildStateSnapshot(character, ssmState, ismState, esState, worldState) {
  ...
  for (const rc of rcList) {
    ...
    // Relationship stage from world state
    ...
    if (esChar) {
      ...
      if (esChar.emotions) {
        for (const [k, v] of Object.entries(esChar.emotions)) {
          if (v > 30) emo.push(`${k}:${v}`)
        }
      }
      if (emo.length) parts.push(emo.join('/'))
    }

    // ISM interaction state
  ...
  lines.push('')
  lines.push('（以上状态必须体现在本轮回复中——不能凭空改变，必须连续演进。）')
...
export async function runAgentTurn(userInput, character, affections, messages, apiKey, onToken, usk) {
  ...
  // ── 📐 STATE SNAPSHOT — always-injected scene + relationship state (HOT zone) ──
  // Builds from SSM/ISM/ES when available, falls back to _worldState data.
  // This ensures the model always knows WHERE we are and WHAT'S happening.
  ...
  // 🔥 v9.1.1 FIX #2: CORE_RECENCY_BLOCK — critical writing rules
